...
# Q5 and Q6
daily_transactions_split = [day.split(';') for day in daily_transactions]
...
for day_transactions in daily_transactions_split:
  transaction = [data_point.strip() for data_point in day_transactions]
...
for color in thread_sold:
  if '&' in color:
    ...
    thread_sold_split.extend(arr_color)
  else:
...
for color in thread_sold_split:
  ...
  print('Thread Shed sold {} threads of {} thread today.'.format(color_count(color), colors[index]))
